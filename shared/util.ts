...
export function sanitizeOptional<T>(
  sanitizer: (x: any, name: string) => T,
  x: any,
  name: string
): undefined|T {
  ...
  try {
    return sanitizer(x, name);
  } catch (e) {
    e.message += 'or undefined';
    throw e;
  }
}

export function sanitizeNumber(x: any, name: string): number {
  if (typeof x !== 'number') {
    throw new Error(`Expected property "${name}" to be of type number`);
  }
